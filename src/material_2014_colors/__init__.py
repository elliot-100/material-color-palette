...
class Color:
    ...
    def __init__(self, name: str, shade: int | str | None = None) -> None:
        ...
        Args:
            ...
            shade:
                A valid shade for `name`.
                If `name` is 'black' or 'white', `shade` is ignored.
        ...
        if name == "black":
            ...
            err_msg = f"Shade must be specified for Material color '{name}'."
            raise ValueError(err_msg)

        elif shade not in _COLORS[name]:
            if (
                isinstance(shade, str)
                and shade.isdigit()
